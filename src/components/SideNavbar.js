      ...
      <NewAppBar position="fixed" open={open}>
        <Toolbar>
          ...
          <Typography variant="h6" noWrap component="div">
            <NewLink to={'/'} delay={1000}>Home</NewLink>
          </Typography>
        </Toolbar>
      </NewAppBar>
      <NewDrawer variant="permanent" open={open}>
        ...
        <List>
          {['SOFT SKILLS', 'HARD SKILLS', 'MY WORKS'].map((text, index) => (
            <ListItem key={text} disablePadding sx={{ display: 'block', overflow:'visible', overflowWrap: 'break-word', marginBottom: '30px'}}>
              <NewLink to={`/${prepareRoute(text)}`} delay={20000}> 
                <ListItemButton
                  sx={{
                    height: 48,
                ...
                {(!open && hover) && 
                <div style={{ display: 'flex', position:'absolute', left:'20%' , bottom: '-34px', fontSize:'12px', overflowWrap: 'break-word', justifyContent: 'center'}}>
                  {text}
                </div>}
            </ListItem>
          ))}
